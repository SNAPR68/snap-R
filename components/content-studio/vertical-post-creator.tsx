...
import { VerticalTemplateRenderer } from './template-renderer'
import { VERTICAL_TEMPLATES, TEMPLATE_CATEGORIES, TemplateDefinition } from '@/lib/content/templates'
...
interface VerticalPostCreatorProps {
  platform: 'story' | 'tiktok'
}

export function VerticalPostCreator({ platform }: VerticalPostCreatorProps) {
  const searchParams = useSearchParams()
  const listingId = searchParams.get('listing')
  const templateRef = useRef<HTMLDivElement>(null)

  const platformName = platform === 'story' ? 'Instagram Story' : 'TikTok'
  const dimensions = { width: 1080, height: 1920 }

  const previewImage = 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=600&h=1000&fit=crop'

  const [selectedTemplate, setSelectedTemplate] = useState<TemplateDefinition>(VERTICAL_TEMPLATES[0])
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `${platform}-${selectedTemplate.id}-${Date.now()}.png`
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold">Create {platformName}</h1>
          <p className="text-white/60">Vertical format optimized for {platform === 'story' ? 'Instagram Stories' : 'TikTok'}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white/5 rounded-xl border border-white/10 p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {VERTICAL_TEMPLATES.map(template => (
                  ...
                  >
                    {/* Mini Template Preview - Vertical */}
                    <div className="w-full relative overflow-hidden" style={{ aspectRatio: '9/16' }}>
                      <div className="absolute inset-0 scale-[0.12] origin-top-left" style={{ width: '833%', height: '833%' }}>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-8">
              ...
              <div className="bg-white/5 rounded-xl border border-white/10 p-4">
                {/* Scaled Preview - Vertical */}
                <div className="relative overflow-hidden rounded-lg mx-auto" style={{ width: '200px', height: '355px' }}>
                  <div 
