...
import { generateRealtorFeed } from '@/lib/syndication/reso-feed';
...
/**
 * GET /api/syndication/realtor
 * Returns Realtor.com compatible RESO XML feed
...
export async function GET(request: Request) {
  ...
  if (listings.length === 0) {
    return new Response(generateRealtorFeed([]), {
      headers: {
  ...
  return new Response(generateRealtorFeed(resoProperties), {
    headers: {
      'Content-Type': 'application/xml; charset=utf-8',
      'Cache-Control': 'public, s-maxage=3600, stale-while-revalidate=86400',
    },
