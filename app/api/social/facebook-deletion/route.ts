...
export async function POST(req: NextRequest) {
  try {
    ...
    // Delete user's social connection data (check result for GDPR compliance)
    ...
    if (deleteError) {
      logger.error('Failed to delete social connections for Facebook user:', deleteError.message);
...
export async function GET(req: NextRequest) {
  ...
  // Validate code is a hex string (as generated by crypto.randomBytes)
  const code = rawCode && /^[0-9a-f]{32}$/.test(rawCode) ? rawCode : null;
  return NextResponse.json({
    status: 'complete',
