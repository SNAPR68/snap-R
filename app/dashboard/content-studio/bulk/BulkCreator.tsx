      ...
      {showResults ? (
        ...
        <div className="flex h-[calc(100vh-56px)]">
          <div className="flex-1 p-6 overflow-auto">
            ...
            : listings.length === 0 ? <div className="text-center py-12 bg-surface rounded-xl border border-white/5"><Home className="w-12 h-12 text-white/10 mx-auto mb-3" /><p className="text-white/40">No listings found</p></div>
            : <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">{listings.map(listing => (
              <button key={listing.id} onClick={() => toggleListing(listing.id)} className={'relative bg-surface rounded-xl border-2 overflow-hidden text-left transition-all ' + (selectedListings.includes(listing.id) ? 'border-purple-500 ring-2 ring-purple-500/30' : 'border-white/5 hover:border-white/20')}>
          ...
          <aside className="w-80 bg-surface border-l border-white/5 flex flex-col">
