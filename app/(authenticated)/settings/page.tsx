        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="md:w-56 flex-shrink-0">
            <nav className="space-y-1">
              {tabs.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-colors ${activeTab === tab.id ? "bg-accent-gold/10 text-primary" : "text-white/60 hover:bg-white/5"}`}>
            ...
            {activeTab === "profile" && (
              <div className="glass-luxury rounded-2xl p-6">
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-gold to-gold-dark flex items-center justify-center text-2xl font-bold text-black">{name?.[0]?.toUpperCase() || user?.email?.[0]?.toUpperCase() || "U"}</div>
                ...
                <div className="mb-6">
                  ...
                  <input type="text" className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/40 focus:outline-none focus:border-primary" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your name" />
                </div>
                <button className="w-full px-4 py-3 bg-gradient-to-r from-gold to-gold-dark rounded-xl text-black font-semibold disabled:opacity-50 flex items-center justify-center gap-2" disabled={loading} onClick={saveProfile}>{loading ? "Saving..." : message ? <><Check className="w-5 h-5" /> Saved</> : "Save Changes"}</button>
              </div>
            )}
            {activeTab === "billing" && (
              <div className="glass-luxury rounded-2xl p-6">
                <h2 className="text-lg font-semibold mb-6">Subscription and Billing</h2>
                <div className="bg-gradient-to-r from-gold/10 to-transparent rounded-xl p-5 border border-accent-gold/30 mb-6">
                  <div className="flex items-center justify-between">
                    <div><p className="text-primary text-sm font-medium">CURRENT PLAN</p><p className="text-2xl font-bold mt-1">{profile?.plan || "Free Trial"}</p></div>
                    <Link href="/billing" className="px-4 py-2 bg-accent-gold text-black rounded-lg font-medium text-sm hover:opacity-90">Upgrade Plan</Link>
                  </div>
                </div>
                <div className="flex items-center justify-between py-4 border-b border-white/10"><div><p className="font-medium">Available Credits</p><p className="text-white/60 text-sm">Used for AI enhancements</p></div><div className="text-2xl font-bold text-primary">{profile?.credits || 0}</div></div>
            ...
            {activeTab === "notifications" && (
              <div className="glass-luxury rounded-2xl p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between py-3"><div><p className="font-medium">Processing Alerts</p><p className="text-white/60 text-sm">Get notified when photos are done</p></div><button onClick={() => setNotifications({...notifications, processingAlerts: !notifications.processingAlerts})} className={`w-12 h-6 rounded-full transition-colors ${notifications.processingAlerts ? "bg-accent-gold" : "bg-white/20"}`}><div className={`w-5 h-5 rounded-full bg-white transition-transform ${notifications.processingAlerts ? "translate-x-6" : "translate-x-1"}`} /></button></div>
                  <div className="flex items-center justify-between py-3 border-t border-white/10"><div><p className="font-medium">Email Updates</p><p className="text-white/60 text-sm">Receive updates about your account</p></div><button onClick={() => setNotifications({...notifications, emailUpdates: !notifications.emailUpdates})} className={`w-12 h-6 rounded-full transition-colors ${notifications.emailUpdates ? "bg-accent-gold" : "bg-white/20"}`}><div className={`w-5 h-5 rounded-full bg-white transition-transform ${notifications.emailUpdates ? "translate-x-6" : "translate-x-1"}`} /></button></div>
                  <div className="flex items-center justify_between py-3 border-t border-white/10"><div><p className="font-medium">Marketing Emails</p><p className="text-white/60 text-sm">Receive tips and offers</p></div><button onClick={() => setNotifications({...notifications, marketingEmails: !notifications.marketingEmails})} className={`w-12 h-6 rounded-full transition-colors ${notifications.marketingEmails ? "bg-accent-gold" : "bg-white/20"}`}><div className={`w-5 h-5 rounded-full bg-white transition-transform ${notifications.marketingEmails ? "translate-x-6" : "translate-x-1"}`} /></button></div>
                </div>
              </div>
            )}
            {activeTab === "security" && (
