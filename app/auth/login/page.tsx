...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    await supabase.auth.signInWithOAuth({ provider: 'google', options: { queryParams: { prompt: 'select_account' }, redirectTo: `${window.location.origin}/auth/callback` } });
  };
  ...
  return (
    <div className="min-h-screen bg-charcoal-deep flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-gold to-gold-dark p-12 flex-col justify-between">
        ...
        <div>
          <h1 className="text-4xl font-bold text-surface mb-4">Welcome Back</h1>
          <p className="text-surface/70 text-lg">Sign in to access your AI-enhanced real estate photos.</p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <button onClick={handleGoogleLogin} className="relative z-10 w-full py-4 px-4 rounded-xl border border-white/20 text-white hover:bg-white/5 flex items-center justify-center gap-3 mb-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <input type="email" placeholder="Email" aria-label="Email address" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-primary" required />
            ...
            <div className="text-right">
              <Link href="/auth/forgot-password" className="text-sm text-white/40 hover:text-primary transition-colors">
                Forgot password?
              </Link>
            </div>
            <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-semibold bg-gradient-to-r from-gold to-gold-dark text-black disabled:opacity-50">
