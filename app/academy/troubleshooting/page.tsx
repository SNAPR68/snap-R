      ...
      <div className="bg-gradient-to-b from-orange-500/20 to-transparent py-12 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center"><HelpCircle className="w-7 h-7 text-on-surface" /></div>
            <div><h1 className="text-3xl font-bold">Troubleshooting</h1><p className="text-on-surface-muted">Common issues and solutions</p></div>
          </div>
        </div>
      </div>
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="space-y-4">
          {articles.map((article, i) => (
            <Link key={i} href={`/academy/troubleshooting/${article.slug}`} className="block bg-surface-container-low rounded-xl p-6 border border-white/10 hover:border-orange-500/50 transition-all group">
              <div className="flex items-start justify-between gap-4">
                <div><h3 className="text-lg font-semibold mb-1 group-hover:text-orange-400">{article.title}</h3><p className="text-on-surface-muted text-sm">{article.description}</p></div>
